        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <docentes-crear v-if="modalMode === 'crear'" @created="nuevo($event)"></docentes-crear>
                    <docentes-editar v-if="modalMode === 'editar'" @updated="modificar($event)"
                        :item="itemSeleccionado"></docentes-editar>
                </div>
            </div>
        </div>
    </div>
...
<script>
import DocenteCrear from './DocenteCrear.vue';
import DocenteEditar from './DocenteEditar.vue';
export default {
    name: 'DocentesList',
    data() {
        return {
    ...
    methods: {
        ...
        nuevo(value) {
            this.axios.post(process.env.VUE_APP_API_URL + '/docentes', value)
                .then((response) => {
                    this.cerrarModal();
                    this.obtenerLista();
                })
        ...
        modificar(value) {
            this.axios.patch(process.env.VUE_APP_API_URL + '/docentes/' + value.id, value)
                .then((response) => {
                    this.cerrarModal();
                    this.obtenerLista();
                })
        ...
        eliminar(value) {
            if (confirm("¿Está seguro de eliminar este ítem?")) {
                this.axios.delete(process.env.VUE_APP_API_URL + '/docentes/' + value.id, value)
                    .then((response) => {
                        this.obtenerLista();
                    })
                    .catch((error) => {
